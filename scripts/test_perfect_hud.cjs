...
const chromeProc = spawn(chromePath, [
  ...
  '--user-data-dir=' + path.resolve('.chrome-pb-perfect-profile')
]);
...
async function main() {
  await wait(2000);
  try {
    const res = await fetch('http://127.0.0.1:9228/json/new?' + encodeURIComponent(fileUrl), { method: 'PUT' });
    ...
    // Apply exact CSS styling
    await send('Runtime.evaluate', {
      expression: `(function() {
        var style = document.createElement('style');
        style.id = 'hud-perfect-css';
        style.textContent = \`
          #hud {
    ...
    fs.writeFileSync('hud_stress_perfect.png', Buffer.from(ssStress.result.data, 'base64'));
    console.log('📸 Saved: hud_stress_perfect.png');

    // Test Normal without PB Time
    await send('Runtime.evaluate', {
      expression: `(function() {
    ...
    fs.writeFileSync('hud_normal_perfect.png', Buffer.from(ssNorm.result.data, 'base64'));
    console.log('📸 Saved: hud_normal_perfect.png');
